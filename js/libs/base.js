...
//mt.currentLenguage = mt.getLanguageURL(); // для мультиязычных сайтов если используется в гостевом интерфейсе
mt.currentLenguage = 'ru'; //Установка языка для панели администрирования так как там не используется сегмент указывающий на язык
...
mt.exist_email = function(emailInput){
	...
	if(user_email != ''){
		if(!mt.isValidEmailAddress(user_email)){$(emailInput).showToolTip(Localize[mt.currentLenguage]['valid_email']);}
		else{
			$.post(mt.baseURL+"valid/exist-email",{'parametr':user_email},
				function(data){if(!data.status){$(emailInput).showToolTip(Localize[mt.currentLenguage]['exist_email']);}},"json");
		}
	}
};
mt.redirect = function(path){window.location=path;}
mt.deleteResource = function(_this){
	if(confirm(Localize[mt.currentLenguage]['confirm']) == false){
		return false;
	}
...
mt.validation = function(jqForm){
	...
	if($(jqForm).find(".valid-phone-number").length == 1){
		...
		if(!mt.isValidPhone($(phoneInput).val().trim())){
			if($(phoneInput).emptyValue() == false){$(phoneInput).setValidationErrorStatus(Localize[mt.currentLenguage]['valid_phone']);}
			errors = true;
		}
	}
...
$.fn.showToolTip = function(ToolTipText){
	...
	var config = {placement:mt.toltipPlacement,trigger:'hover',title:ToolTipText}
	return this.each(
		function(){
