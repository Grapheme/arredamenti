			...
			<div class="span9">
				...
				<table class="table table-bordered" data-action="<?=site_url(ADMIN_START_PAGE.'/pages/remove');?>">
					...
					<?php for($i=0;$i<count($pages);$i++):?>
						<tr>
							<td><?=$pages[$i]['ru_page_title'].' ('.$pages[$i]['en_page_title'].')';?></td>
							<td>
								<a href="<?=site_url(ADMIN_START_PAGE.'/pages/'.$pages[$i]['url'].'/edit?mode=text&id='.$pages[$i]['id'])?>" class="btn btn-link" ><i class="icon-edit"></i></a>
							</td>
						</tr>
					<?php endfor;?>
					</tbody>
