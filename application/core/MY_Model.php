...
class MY_Model extends CI_Model{
	...
	function getWhere($primary_key = NULL,$where = NULL,$manyRecords = FALSE,$group_by = NULL){
		
		if(!is_null($primary_key)):
			$this->db->select($this->_fields());
			$this->db->order_by($this->order_by);
			if(!is_null($group_by) && is_array($group_by)):
	...
	function limit($limit = NULL,$offset = NULL,$orderby = NULL,$where = NULL){
	...
	function search($search_field = NULL,$search_parameter = NULL){
	...
	function updateField($primary_key = NULL,$field = NULL,$value = NULL){
		
		if(!is_null($primary_key) || !is_null($field) || !is_null($value)):
			$this->db->set($field,$value)->where($this->primary_key,$primary_key)->update($this->table);
	...
	function getCountRecord($where = NULL,$group_by = NULL,$fields = NULL,$manyRecords = FALSE){
		
		if(!is_null($fields)):
			$this->db->select('COUNT(*) AS count ,'.$fields);
		else:
	...
	function getNextNumber($where = NULL,$field = 'number'){
		
		$this->db->select_max($field,'max_number');
		if(!is_null($where) && is_array($where)):
			$this->db->where($where);
		endif;
		$query = $this->db->get($this->table);
