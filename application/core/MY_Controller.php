...
class MY_Controller extends CI_Controller{
	...
	public function parseAndSendMail($langURL,$parserData){
		...
		if(isset($parserData['email']) && !empty($parserData['email'])):
			...
			return $this->sendMail('info@arrendamenti.su','no-reply@arredamenti.su ','arrendamenti.su','Контактная форма',$mailtext);
		else:
			return FALSE;
		endif;
	}
	...
	public function uploadImages($uploadPath = NULL, $files_array_key = false){

		$uploadStatus = array('status'=>FALSE,'message'=>'','uploadData'=>array());
		if (!$files_array_key) {			$uploadStatus['message'] = "Bad \$_FILES array key";
		}
		if(is_null($uploadPath) || ($this->createDir($uploadPath) === FALSE)):
			$uploadPath = NULL;
		endif;
		if(!is_null($uploadPath)):
			if(!empty($_FILES) && count($_FILES[$files_array_key]['name'])):

				#print_r($_FILES[$files_array_key]); #die;
				foreach ($_FILES[$files_array_key]['name'] as $f => $file) {
    				...
    				$config['overwrite'] = TRUE;
    				$config['max_size'] = 5120;
    				$config['file_name'] = random_string('nozero',12).'.'.substr(strrchr($file, '.'),1);
    				$this->upload->initialize($config);

					#print_r($files_array_key . $f);    				if(!$this->upload->do_upload($files_array_key . $f)):
    					$uploadStatus['message'] = $this->load->view('html/print-error',array('alert_header'=>'Файл: '.$file,'message'=>$this->upload->display_errors()),TRUE);
    				else:
    					$uploadStatus['uploadData'] = $this->upload->data();
    					$uploadStatus['status'] = TRUE;
    				endif;
